...
static const PHYSFS_Io ZIP_Io = { CURRENT_PHYSFS_IO_API_VERSION, NULL, ZIP_read, ZIP_write, ZIP_seek, ZIP_tell, ZIP_length, ZIP_duplicate,
    ZIP_flush, ZIP_destroy };
...
static ZIPentry* zip_load_entry(ZIPinfo* info, const int zip64, const PHYSFS_uint64 ofs_fixup)
{
    ...
    BAIL_IF_ERRPASS(!readui32(io, &deps_attr), NULL);
    BAIL_IF_ERRPASS(!readui32(io, &ui32), NULL);
    ...
    memcpy(((PHYSFS_uint8*)retval) + sizeof(__PHYSFS_DirTreeEntry), ((PHYSFS_uint8*)&entry) + sizeof(__PHYSFS_DirTreeEntry),
    ...
    if(isdir)
        ...
        retval->resolved = (zip_has_symlink_attr(retval, deps_attr)) ? ZIP_UNRESOLVED_SYMLINK : ZIP_UNRESOLVED_FILE;
    } /* else */
    ...
    if((zip64)
        && ((offset == 0xFFFFFFFF) || (starting_disk == 0xFFFFFFFF) || (retval->compressed_size == 0xFFFFFFFF)
...
static int zip_load_entries(ZIPinfo* info, const PHYSFS_uint64 data_ofs, const PHYSFS_uint64 central_ofs, const PHYSFS_uint64 entry_count)
{
    PHYSFS_Io* io = info->io;
...
static PHYSFS_sint64 zip64_find_end_of_central_dir(PHYSFS_Io* io, PHYSFS_sint64 _pos, PHYSFS_uint64 offset)
{
    /*
...
static int zip64_parse_end_of_central_dir(ZIPinfo* info, PHYSFS_uint64* data_start, PHYSFS_uint64* dir_ofs, PHYSFS_uint64* entry_count,
    PHYSFS_sint64 pos)
{
    PHYSFS_Io* io = info->io;
    PHYSFS_uint64 ui64;
...
static int zip_parse_end_of_central_dir(ZIPinfo* info, PHYSFS_uint64* data_start, PHYSFS_uint64* dir_ofs, PHYSFS_uint64* entry_count)
{
    PHYSFS_Io* io = info->io;
...
const PHYSFS_Archiver __PHYSFS_Archiver_ZIP = { CURRENT_PHYSFS_ARCHIVER_API_VERSION,
    {
        "ZIP",
    ...
    ZIP_openArchive, __PHYSFS_DirTreeEnumerate, ZIP_openRead, ZIP_openWrite, ZIP_openAppend, ZIP_remove, ZIP_mkdir, ZIP_stat,
    ZIP_closeArchive };
