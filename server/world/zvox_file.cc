...
constexpr static std::uint8_t ZVOX_MAGIC_1 = 0x5A;
constexpr static std::uint8_t ZVOX_MAGIC_2 = 0x56;
constexpr static std::uint8_t ZVOX_MAGIC_3 = 0x4F;
...
bool ZvoxFile::is_empty(void) const
{
...
bool ZvoxFile::is_valid(void) const
{
    return m_valid;
}

bool ZvoxFile::read_slot(std::size_t slot, ReadBuffer& buffer)
{
...
void ZvoxFile::write_slot(std::size_t slot, const WriteBuffer& buffer)
{
    if(!m_valid || slot >= constant::REGION_VOLUME) {
        return;
    }
...
bool ZvoxFile::load_header(void)
{
    ...
    header_valid = header_valid && magic_1 == ZVOX_MAGIC_1;
    header_valid = header_valid && magic_2 == ZVOX_MAGIC_2;
    header_valid = header_valid && magic_3 == ZVOX_MAGIC_3;
    header_valid = header_valid && magic_4 == ZVOX_MAGIC_4;
    header_valid = header_valid && version <= ZVOX_VERSION;
...
void ZvoxFile::save_header(void)
{
    WriteBuffer buffer;

    buffer.write<std::uint8_t>(ZVOX_MAGIC_1);
    buffer.write<std::uint8_t>(ZVOX_MAGIC_2);
    buffer.write<std::uint8_t>(ZVOX_MAGIC_3);
    buffer.write<std::uint8_t>(ZVOX_MAGIC_4);
    buffer.write<std::uint32_t>(ZVOX_VERSION);
